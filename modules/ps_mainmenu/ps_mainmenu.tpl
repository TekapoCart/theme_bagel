    ...
    {if $nodes|count && $depth < 2}
        <ul class="{*top-menu*}" {if $depth == 0}id="top-menu"{/if} data-depth="{$depth}">
            {foreach from=$nodes item=node}
                {if $depth ===1 && $node.type === 'cms-page'}{continue}{/if}
                <li class="{$node.type}{if $node.current || $currentCategoryPageIdentifier == $node.page_identifier} current{/if}{if $c_tree_path && in_array($node.page_identifier, $c_tree_path)} on{/if}" id="{$node.page_identifier}">
                    ...
                    >{if $node.children|count}{* Cannot use page identifier as we can have the same page several times *}
                            {assign var=_expand_id value=10|mt_rand:100000}
                            <span class="float-xs-right hidden-md-up">
                                <span data-target="#top_sub_menu_{$_expand_id}" data-toggle="collapse" class="navbar-toggler collapse-icons">
                                <i class="material-icons add">&#xE313;</i>
                                <i class="material-icons remove">&#xE316;</i></span></span>
                        {/if}{$node.label}{* <br>a:{$node.current} <br>b:{$currentCategoryPageIdentifier} <br>c:{$node.page_identifier}*}
                        <span class="hidden-sm-down">{*if $node.children|count && $depth === 0}
                        <i class="material-icons expand-more">&#xE5C5;</i>
                        {/if*}</span></a>
                    {if $node.children|count && ($node.current ||
                        (($c_tree_path && in_array($node.page_identifier, $c_tree_path)) ||
                        ($c_tree_path && in_array($currentCategoryPageIdentifier, $c_tree_path)))
                    )}
                        <div {if $depth === 0} class="subnav{*popover sub-menu js-sub-menu collapse*}{if $c_tree_path && in_array($node.page_identifier, $c_tree_path)} in{/if}"{else} class="collapse{if $c_tree_path && in_array($node.page_identifier, $c_tree_path)} in{/if}"{/if} id="top_sub_menu_{$_expand_id}" aria-expanded="{if $c_tree_path && in_array($node.page_identifier, $c_tree_path)}true{else}false{/if}">
